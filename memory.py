...
class Memory:
    ...
    def _check_addr(self, address):
        ...
        if address < 0:
            raise ValueError(f"Address {address:#04X} is negative")
        if address > STACK_TOP:
            raise ValueError(
                f"Address {address:#04X} is in reserved memory "
                f"address must be less than {STACK_BASE:#04X}"
            )
        return True
